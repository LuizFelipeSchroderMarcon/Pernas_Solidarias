...
export const Badge: React.FC<BadgeProps> = ({
  ...
  const variants: Record<BadgeVariant, string> = {
    ...
    info: 'bg-indigo-50 text-indigo-700 border-indigo-200 dark:bg-indigo-950/60 dark:text-indigo-300 dark:border-indigo-800',
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 font-semibold rounded-full border leading-none ${variants[variant]} ${sizes[size]} ${className}`}
    >
      {children}
    </span>
  );
